...
class BopBaseReader:
    ...
    def get_depth(self, i, filled=False):
        if filled:
            ...
            # depth = cv2.imread(depth_file, -1) * self.bop_depth_scale   # tmp
        if self.resize != 1:
    ...
    def get_mask(self, i_frame: int, ob_id: int, type='mask_visib'):
        ...
        if self.scene_gt is not None:
            ...
            if not os.path.exists(mask_file):
                print(f"[DEBUG] Mask file not found: {mask_file}")
                return None
        else:
            raise RuntimeError
        ...
        print(
            f"[DEBUG] Mask loaded for frame {i_frame}, ob_id {ob_id}: shape {mask.shape}, unique values {np.unique(mask)}")
        return mask > 0
    ...
    def get_gt_mesh(self, mesh_dir: str):
        ...
        mesh.vertices *= 1e-3     # 这里为什么要乘1e-3(因为要统一成m单位，不乘的话现在就是mm单位)
        return mesh
