...
def create_sam_processor(cad_path, sam_tmp_dir, segmentor_model, stability_score_thresh):
    return RealTimeProcessor(cad_path, sam_tmp_dir, segmentor_model, stability_score_thresh)

@timed("初始化FP")
def create_fp_processor(model_path, camera_path):
    return FoundationposeProcessor(model_path, camera_path)
...
def capture_and_process(cad_path, sam_tmp_dir, data_dir, segmentor_model, stability_score_thresh, use_parallel_mode):
    # 清空并创建输出目录
    ...
    # # 删除旧目录并重新创建
    for dir_path in [rgb_dir, depth_dir, mask_dir, track_dir]:
    ...
    frames = pipeline.wait_for_frames() # 读取初始帧数以读取相机内参
    color_frame = frames.get_color_frame()
    # depth_frame = frames.get_depth_frame()  # 获得depth
    intr = color_frame.profile.as_video_stream_profile().intrinsics
    cam_info = {
        "cam_K": [intr.fx, 0, intr.ppx, 0, intr.fy, intr.ppy, 0, 0, 1],
        "depth_scale": 0.001  # 保存的单位是m(暂时与已知pipeline统一，后期可修改)
    }
    # 将相机内参保存到 JSON 文件
    with open('Data/my_data/camera.json', 'w') as json_file:
        json.dump(cam_info, json_file, indent=4)

    # 将深度图对齐到彩色图的流的坐标系
    align_to = rs.stream.color
    align = rs.align(align_to)

    if use_parallel_mode:   # 显存中存储
        # 初始化SAM处理实例
        sam_processor = create_sam_processor(cad_path, sam_tmp_dir, segmentor_model, stability_score_thresh)

        # 初始化foundation处理实例
        fp_processor = create_fp_processor(model_path=cad_path, camera_path=os.path.join(data_dir, 'camera.json'))

        frame_count = 0 # 初始化计数器
        try:
            while True:
                # 需要拍摄多张，否则只拍摄一张的话,结果容易出问题
                for i in range(10):
                    frames = pipeline.wait_for_frames()  # 捕获帧
                    aligned_frames = align.process(frames)
                    depth_frame = aligned_frames.get_depth_frame()  # 获取对齐的数据
                ...
                # 保存帧到指定文件
                rgb_path = os.path.join(rgb_dir, f"{frame_count}.png")
                depth_path = os.path.join(depth_dir, f"{frame_count}.png")
                mask_path = os.path.join(mask_dir, f"{frame_count}.png")
                cv2.imwrite(rgb_path, np.asanyarray(color_frame.get_data()))
                cv2.imwrite(depth_path, np.asanyarray(depth_frame.get_data()))
                ...
                cv2.imwrite(os.path.join(track_dir, f'{frame_count}.png') , vis_img[..., ::-1])
                print(f"检测帧{frame_count}.png已保存")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cad_path", default="Data/Instantmesh/Instantmesh_mm.ply", help="Path to CAD model")  # 注意输入单位为mm，中间处理为m
    parser.add_argument("--sam_tmp_dir", default="Data/Instantmesh/sam_tmp", help="Output directory")
    parser.add_argument("--data_dir", default="Data/Instantmesh", help="rgb, depth and mask")
    parser.add_argument("--segmentor_model", choices=["sam", "fastsam"], default="fastsam", help="Segmentor model type")
