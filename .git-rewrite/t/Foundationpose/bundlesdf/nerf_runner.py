...
class NerfRunner:
  def __init__(self,cfg,images,depths,masks,normal_maps,poses,K,_run=None,occ_masks=None,build_octree_pcd=None):
    ...
    # self.vis.create_window(window_name="Real-time Mesh Visualization")
  ...
  def visualize_3d_model(self):
    # 提取当前模型
    mesh = self.extract_mesh(isolevel=0, voxel_size=0.003)
    if mesh is None:
    ...
    # 实时显示 3D 模型
    o3d.visualization.draw_geometries([o3d_mesh], window_name=f"Step {self.global_step}")
  ...
  def train_loop(self,batch):
    ...
    # 统计 trunc 范围内的点的数量
    # sdf = extras['raw'][..., -1]  # 提取 SDF 值
    # z_vals = extras['z_vals']  # 提取 z 值
    # truncation = self.get_truncation()  # 获取当前 truncation
    # trunc_mask = torch.abs(sdf) < truncation  # 判断哪些点的 sdf 在 trunc 范围内
    # num_points_in_trunc = trunc_mask.sum().item()  # 计算满足条件的点的数量
    # # 打印统计信息
    # print(f"Step {self.global_step}: {num_points_in_trunc} 个点在trunc范围内")

    # 每隔一定步数更新可视化
    # if self.global_step % 500 == 0:  # 每 100 步进行一次模型提取和绘制
  ...
  def mesh_texture_from_train_images(self, mesh, rgbs_raw, tex_res=1024):
    ...
    # debug
    tex_image = tex_image / (weight_tex_image[..., None] + 1e-8)
    ...
    try:
      ...
      # 检查是否存在非法值（NaN 或 Inf）
      if np.isnan(tex_image).any() or np.isinf(tex_image).any():
        print("Warning: Detected NaN or Inf in tex_image during texture computation.")
        print(f"tex_image stats: min={np.nanmin(tex_image)}, max={np.nanmax(tex_image)}, mean={np.nanmean(tex_image)}")

        # 替换非法值
        tex_image = np.nan_to_num(tex_image, nan=0, posinf=255, neginf=0)

      # 确保值在 0-255 范围内，并转换为 uint8
      tex_image = np.clip(tex_image, 0, 255).astype(np.uint8)

    except Exception as e:
      # 捕获异常并输出调试信息
      print(f"Error in processing tex_image: {e}")
      print(f"tex_image stats: min={np.nanmin(tex_image)}, max={np.nanmax(tex_image)}, mean={np.nanmean(tex_image)}")
      raise
