...
def visualize_mask(detections, save_path="mask.png"):
    """
       ...
       参数:
           detections: 检测结果，包含 segmentation 信息。
           save_path: 保存路径。
    ...
    if best_det is None:
        raise ValueError("No detections found to visualize!")
...
def run_inference(segmentor_model, output_dir, cad_path, rgb_path, depth_path, cam_path, stability_score_thresh):
    ...
    # 进行输入RGB和掩码与分割结果的特征匹配和分数计算
    (
        idx_selected_proposals,
        pred_idx_objects,
        semantic_score,
        best_template,
    ) = model.compute_semantic_score(query_decriptors)  # 语义分数
    ...
    # 使用 visualize_mask 生成掩码图像
    mask_save_path = f"{output_dir}/sam6d_results/mask.png"
    visualize_mask(detections, save_path=mask_save_path)
    logging.info(f"Mask saved to {mask_save_path}")
